...
export default defineComponent({
  ...
  setup(props) {
    ...
    const ensureLeafletCss = () => {
      const already = Array.from(document.styleSheets).some((s) =>
        (s.href || '').includes('leaflet'),
      )
      if (!already) {
    ...
    onMounted(() => {
      initializeMap()
      ;(window as unknown as { deleteMarked: (id: string) => void }).deleteMarked =
  ...
  <div :class="isExpanded ? 'map-wrapper-expanded' : 'map-wrapper'">
    ...
    <div :class="isExpanded ? 'map-overlay-panel-expanded' : 'map-overlay-panel'">
      <div style="display: flex; gap: 8px; flex-wrap: wrap">
        ...
        <button
          @click.prevent="toggleMarkMode"
          :style="{
            padding: '8px 12px',
            background: markMode ? '#9C27B0' : 'white',
            color: markMode ? 'white' : 'black',
          }"
        >
          📌 Marcar
        </button>
      ...
      >
        ...
        <div v-else>
          ...
          <div style="color: #cc0000; font-weight: bold">
            <b>📍 Próximas a fiação:</b> {{ crowdedTreeCount }}
          </div>
      ...
      <div style="margin-top: 6px; font-size: 11px; color: #666">
        ...
        <span v-if="markMode" style="color: #9c27b0; font-weight: bold"
          >| Modo marcação ativado</span
        >
      </div>
    </div>
  </div>
